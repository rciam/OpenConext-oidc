...
public class DynamicClientRegistrationEndpoint {
	...
	private ClientDetailsEntity validateScopes(ClientDetailsEntity newClient) throws ValidationException {
		...
		if (newClient.getGrantTypes().contains("urn:ietf:params:oauth:grant-type:token-exchange") && !newClient.getGrantTypes().contains("authorization_code")) {
			allowedScopes = scopeService.removeUndefinedScopes(requestedScopes);
		}

		// if the client didn't ask for any, give them the defaults
		if ((allowedScopes == null || allowedScopes.isEmpty()) && newClient.getGrantTypes().contains("urn:ietf:params:oauth:grant-type:token-exchange") && !newClient.getGrantTypes().contains("authorization_code")) {
			allowedScopes = scopeService.getDefaults();
		} else if ((allowedScopes == null || allowedScopes.isEmpty())) {
			//allowedScopes = scopeService.getDefaults();
			allowedScopes = scopeService.getNoRestrictedDefaults();
		}
	...
	private ClientDetailsEntity validateGrantTypes(ClientDetailsEntity newClient) throws ValidationException {
		...
		if (newClient.getGrantTypes().contains("urn:ietf:params:oauth:grant-type:token-exchange")) {
	...
	private ClientDetailsEntity validateRedirectUris(ClientDetailsEntity newClient) throws ValidationException {
		...
		if (newClient.getGrantTypes().contains("authorization_code") || newClient.getGrantTypes().contains("implicit") || newClient.getGrantTypes().contains("urn:ietf:params:oauth:grant-type:token-exchange")) {
